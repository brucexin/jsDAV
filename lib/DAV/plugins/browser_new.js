...
//        "icons/calendar": jsCalDAV_iCalendar,
    ...
    initialize: function(handler) {
        ...
        console.log("handler.server.lang "+handler.server.lang);
        if(handler.server.lang == "cn") {
          var tplstr = Fs.readFileSync(Path.join(__dirname, 'tpl/folder_cn.ejs'), {encoding:'utf8'});
        } else {
          var tplstr = Fs.readFileSync(Path.join(__dirname, 'tpl/folder.ejs'), {encoding:'utf8'});
        }
        this.tpl = Ejs.compile(tplstr);
    ...
    httpGetInterceptor: function(e, method) {
        ...
        var uri  = decodeURI(this.handler.getRequestUri());
        console.log("GetInterceptor:"+uri);
        var self = this;
        this.handler.getNodeForPath(uri, function(err, node) {
            if(err) {
              console.log("getNodeForPath failed:"+err);
            }
            ...
            self.generateDirectoryIndex(uri, function(err, sIndex) {
                if (err) {
                  console.log("generateDirectoryIndex got error:"+err);
                  return e.next(err);
                }
                e.stop();
    ...
    generateDirectoryIndex: function(path, cbindex) {
        var self = this;
        var data =  {
          title: this.escapeHTML(path) + "/ - jsDAV " + jsDAV_Server.VERSION,
          current_path: this.escapeHTML(path),
          ...
          function(cb) {
            self.handler.getNodeForPath(path, function(err, parent) {
              if (err)
                ...
                //if path empty, meaning now we at root node
                if(path) {
                  data.hasParent = true;
                  data.parentIcon = self.enableAssets ? self.getAssetUrl("icons/parent" + self.iconExtension) : "";
                  data.parentUri = Util.splitPath(path)[0];
                  data.parentFullPath = encodeURI(self.handler.server.getBaseUri() + data.parentUri);
                } else {
            ...
            Async.list(Object.keys(files))
              .each(function(filename, next) {
                ...
                if (Util.rtrim(file["href"], "/") == path)
                    return next();
                makeFileItem(self, path, file, filename, function(error, item) {
                  if(error) {
                    console.log("makeFileItem failed:"+error);
                  } else if(item) {
                    items.push(item);
                  }
                  return next();
            ...
            //self.handler.getNodeForPath(path, function(err, parent) {
                ...
                //if (path) {
                    ...
                    //icon = self.enableAssets
                        //? '<a href="' + fullPath + '"><img src="' + self.getAssetUrl("icons/parent" + self.iconExtension) + '" width="24" alt="Parent" />'+parentUri+'</a>'
                        //: "";
                    //html += "<tr>\n\
//<td>" + icon + "</td>\n\
...
//</tr>";
                ...
                //Async.list(Object.keys(files))
                    //.each(function(filename, next) {
                        ...
                        //var originName = Util.splitPath(file["href"])[1];
                        //name = encodeURI(originName || "");
                        //console.log("name:"+name+" origin:"+file["href"]);
...
function makeFileItem(browser, path, file, filename, cb) {
  try {
    var self = browser;
    var originName = Util.splitPath(file["href"])[1];
    var name = encodeURI(originName || "");
    console.log("name:"+name+" origin:"+file["href"]);

    var type = formatResourceType(file);
    type = self.escapeHTML(type.replace(/;.*$/, ""));

    var size = file["200"]["{DAV:}getcontentlength"];
    if (!size || isNaN(parseInt(size)))
      size = "";
    var filedate = file["200"]["{DAV:}getlastmodified"].getTime();
    var lastmodified = !Util.empty(file["200"]["{DAV:}getlastmodified"])
      //? Util.dateFormat(file["200"]["{DAV:}getlastmodified"].getTime(), Util.DATE_RFC822)
      ? Util.dateFormat(filedate, Util.DATE_ISODATE)+" "+Util.dateFormat(filedate, Util.DATE_ISOTIME)
      : "";

    var fullPath = encodeURI("/" + Util.trim(self.handler.server.getBaseUri() + (path ? path + "/" : "") + name, "/"));
                      
    var displayName = typeof file["200"]["{DAV:}displayname"] == "string"
                  ? file["200"]["{DAV:}displayname"]
                  : originName;
  } catch(e) {
  ...
  if (self.enableAssets) {
    self.handler.getNodeForPath((path ? path + "/" : "") + originName, function(err, node) {
      console.log("get node info "+(path ? path + "/" : "") + originName);
      if (err) {
        console.log("getNodeForPath failed:"+err);
        return cb(null, {
          icon: icon,
      ...
      for (var iconName in self.iconMap) {
        if (node.hasFeature(self.iconMap[iconName])) {
          icon = self.getAssetUrl(iconName + self.iconExtension);
          break;
        }
      }
      console.log("icon is "+icon);
      var _displayName = node.getDisplayName();
      return cb(null, {
            icon: icon,
            type: type,
            collection: type == 'Collection',
...
function formatResourceType(file) {
  var type;
  if (file["200"]["{DAV:}resourcetype"]) {
    type = file["200"]["{DAV:}resourcetype"].getValue();
    // resourcetype can have multiple values
