...
var jsDAV_FS_Directory = module.exports = jsDAV_FS_Node.extend(jsDAV_Collection, jsDAV_iQuota, {
    initialize: function(path) {
        this.path = Util.regularPath(path);
        console.log("directory path:"+this.path);
    },
    ...
    getChildren: function(cbfsgetchildren) {
        ...
        //console.log("paths:"+Fs.readdirSync(this.path));
/*       Fs.readdir(this.path, function(err, files) {*/
          //if(err) {
            //return cbfsgetchildren(err);
          //} else {
            //Async.each(function(file, cbnextdirch) {
              //console.log("get stat:"+file);
              //Fs.stat(file, function(err, Stats) {
                //if(err) {
                  //console.log("stat error:"+err);
                  //return cbnextdirch(err);
                //} else {
                  //return cbnextdirch({'path':file, 'stat':Stats});
                //}
              //});
            //}).end(function() {
       ...
       Async.readdir(this.path)
             ...
             .each(function(file, cbnextdirch) {
                 console.log("getChildren:"+file.path);
                 Fs.stat(file.path, function(err, stat) {
                   if(err) {
                     console.log("stat got error:"+err+" ,ignore it!");
                     return cbnextdirch();
                   } else {
                     file.stat = stat;
